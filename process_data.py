...
def chooseBestFit(params, errors, threshold):
	bestFits = {}
...
def getFitData(root_path, out_path, threshold):

	fitData = open(out_file_name, 'w')

	# Each row of allData has data for one split time series
	allData = []

	for dirpath, dirnames, filenames in os.walk(root_path):
		for filename in [f for f in filenames if f.endswith("Actual.csv")]:
			print filename
			allData.append(   split_timeseries(  get_timeseries(root_path + filename) ) )
	...
	for j in range(len(allData)):
		...
		for i in range(len(data)):
			...
			params, errors = getAllFitParameters(data[i])
			fit, p, e = chooseBestFit(params, errors, threshold)

			fit_info = []
			fit_info.append(fit)
			for parameter in p:
				fit_info.append(parameter)
			fit_info.append(e)
	...
	print 'Done collecting fit data'
	return

if __name__ == "__main__":
	root_path = '/Users/abalakrishna/Desktop/data/Influx Data - JPL/'
	out_file_name = 'best_fit_data.csv'
	threshold = 0.20
	getFitData(root_path, out_file_name, threshold)
